...
class TransaksiProduct extends BaseController
{
    ...
    public function TransaksiID($unit_id)
    {
        ...
        $SQL = "SELECT MAX(RIGHT(no_transaksi, 4)) as kode FROM transaksi_product WHERE unit_id ='" . $unit_id . "' AND date='" . date('Y-m-d') . "'";
    ...
    public function index()
    {
        ...
        if ($this->UserInfo->level == 'Admin Cabang') {
            ...
            $TransaksiProduct = $this->db->table("transaksi_product")->where("unit_id", $this->UserInfo->unit_id)->where("date", date('Y-m-d'))->get()->getResultObject();
            $data             = [
    ...
    public function pay_transaksi_detail()
    {
        ...
        $this->db->table("transaksi_product_detail")->insert($data);
    }
    ...
    public function pay_debit()
    {
        ...
        $query = $this->db->table("transaksi_product")->insert($data);
        if ($query) {
            $response = [
                'status'  => 'success',
                'message' => "success",
    ...
    {
        $no_transaksi     = $this->request->getPost('no_transaksi');
        $data_transaksi   = $this->db->table('transaksi_product')->where('no_transaksi', $no_transaksi)->get()->getRowObject();
        $detail_transaksi = $this->db->table('transaksi_product_detail')->where('no_transaksi', $no_transaksi)->get()->getResultObject();
    ...
    public function delete()
    {
        ...
        $no_transaksi = $this->request->getPost("no_transaksi");
        $query1       = $this->db->table('transaksi_product')->where('no_transaksi', $no_transaksi)->delete();
        $query2       = $this->db->table('transaksi_product_detail')->where('no_transaksi', $no_transaksi)->delete();
        if ($query1 && $query2) {
            $response = [
                'status'  => 'success',
                'message' => 'success',
    ...
    public function invoice()
    {
        ...
        $value_row  = $this->db->table("transaksi_product")->where("no_transaksi", $no_trans)->get()->getRowObject();
        $value_prod = $this->db->table("transaksi_product_detail")->where("no_transaksi", $no_trans)->get()->getResult();
